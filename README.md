...
This repo is also configured for Cloudflare Pages using [`wrangler.toml`](./wrangler.toml).

One-time setup:

1. Create or choose a Cloudflare Pages project named `fire-planner`
...
- the production bundle is still generated by `npm run build`
- on the first deploy, Wrangler may prompt you to create or link the Pages project if it does not already exist
